...
> 为什么 Mesh 只管存储、不管渲染：Mesh 是纯数据容器（和 Framebuffer 类似）。它不知道 MVP 矩阵、不知道着色器、不知道光栅化。把数据（Mesh）和逻辑（Pipeline）分开，是[`plan.md`](docs/plan.md)中"数据的归数据，逻辑的归逻辑"原则的体现。改渲染算法只改 Pipeline，改模型数据只改 Mesh——互不干扰。
