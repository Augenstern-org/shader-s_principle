...
6. **Release 构建**（可选）：确认无编译警告

### 实际实现与原计划的差异

与原计划基本一致（提交 `ea00206`）。`createDemoMesh()` 绕序无需调整——两个三角形在默认视角下均为正面朝向。
...
1. **AOV Bézier 曲面支持（原计划外）**：除 tinyobjloader 标准 OBJ 加载外，新增自定义 AOV 格式解析器（`isAOVFormat`、`parseAOV`、`buildMeshFromPatches`），支持 Utah 茶壶等双三次 Bézier 曲面模型。使用伯恩斯坦多项式在 8×8 网格上细分曲面片，生成索引三角形。

2. **`loadStandardObj` 按面创建顶点（不复用）**：原计划使用 `addVertex` + `addIndexedTriangle` 支持顶点复用。实现中每个面仍创建独立顶点（即使位置共享），因为 OBJ 中同一位置可能在不同面上有不同法线/UV（纹理接缝、硬边），共用顶点会导致插值错误。

3. **`Mesh::createFallbackMesh` 新增**：加载失败时生成默认三角形作为回退，避免黑屏。

4. **更改自动旋转为`Space`键**：原先是`M`键。
---

## 阶段 8：纹理映射

**日期**：2026-05-28（规划），2026-05-28（实现）  
**状态**：[x] 已实现
...
1. **Shader.h 直接 include Texture.h**：原计划用前向声明 `class Texture;` 避免头文件依赖。实现中直接 `#include "Texture.h"`——因为 `Uniforms` 中持有 `const Texture*` 指针，前向声明已足够，但完整 include 也正确且更简单。

2. **`Texture()` / `~Texture()` 使用 `= default`**：原计划写空函数体。实现中成员变量均有默认初始值（`m_width = 0`、`m_height = 0`、空 `vector`），编译器生成的默认版本已满足需求，`= default` 更简洁。

3. **`BuiltinVertexShader::test` 未透传 texcoord**：`test` 着色器使用双参数 `VertexOutput` 构造函数，texcoord 默认 `(0,0)`——该着色器仅用于调试，不影响正常渲染路径。

4. **ObjLoader texcoord 提取 bug 修复**：实现中发现并修复了两个 bug——`attrib.texcoords` 索引计算错误（`2 +` 误写为 `2 *`）和三个顶点共用同一 texcoord（`getTexcoord(i0)` 重复三次）。

5. **纹理加载失败回退**：原计划 `loadFromFile` 失败时直接退出。实现中改为 stderr 警告 + 继续运行——片段着色器 `texture` 在 `texture == nullptr || !valid()` 时回退到顶点颜色，程序不会崩溃。
