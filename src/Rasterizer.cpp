...
namespace Rasterizer {
    ...
    void rasterizeTriangle(
        ...
        glm::vec2 t0_div_w = glm::vec2(out0.texcoord) * inv_w0;
        glm::vec2 t1_div_w = glm::vec2(out1.texcoord) * inv_w1;
        glm::vec2 t2_div_w = glm::vec2(out2.texcoord) * inv_w2;
        ...
        for (int y = minY; y <= maxY; ++y) {
            for (int x = minX; x <= maxX; ++x) {
                ...
                if (e1 >= 0 && e2 >= 0 && e3 >= 0) {
                    ...
                    glm::vec2 inTexcoord = texcoord_over_w / inv_w;
                    Color finalColor = fragShader.process(FragmentInput(inColor, inTexcoord), uniforms);
